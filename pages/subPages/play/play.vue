			...
			<view class="content">
				...
				<view class="play-poster">
					<view class="poster-box" :class="playState ? 'play' : 'pause' ">
						<image class="img" :src="curSongItem.picUrl" mode=""></image>
					</view>
				</view>
				<view class="bot">
					<!-- 上排的图标 -->
					...
					<view class="slider-bar flex-box">
						...
						min="0" block-size="15" @change="sliderChange" backgroundColor="rgba(255,255,255,.5)"
					...
					<view class="btn-groups flex-box">
						...
						<view class="flex-item" @click="setPlayModel()">
							<!-- 列表循环 -->
							<view class="iconfont" v-if="playModel==0">
								&#xe66c;
							</view>
							<!-- 随机 -->
							<view class="iconfont" v-if="playModel==1">
								&#xe66b;
							</view>
							<!-- 单曲 -->
							<view class="iconfont" v-if="playModel==2">
								&#xe66d;
							</view>
						</view>
						<!-- 上一曲 -->
...
<script>
	import {apiGetSongDetail,apiGetSongUrl} from  '../../../apis/index.js'
	...
	export default {
		data() {
			return {
				...
				curSongItem:{},			//当前播放对象
				playTime:0,			//音乐播放时长
				playModel:0			//播放模式  0-代表列表循环	1-随机 2-单曲	
			}
		},
		computed:{
			...mapState(['playList']),
		...
		methods: {	
			...
			//获取音乐文件 详情 同时调用多个接口
				...
				//获取音乐时长，可能读取的时候innerAudioContext还没有准备好
				let timer = setInterval(()=>{
					console.log('时长',innerAudioContext.duration);
						//如果innerAudioContext.duration时长是NAN
			...
			nextPlay(preview){
				...
				if(this.playModel == 0){
					if(!preview){
						this.curPlayIndex = this.curPlayIndex >= 
						(this.audioList.length-1) ? 0 : this.curPlayIndex + 1
					} else{
						this.curPlayIndex = (this.curPlayIndex<1) ?
						 this.audioList.length -1 :this.curPlayIndex -1
					}
				}
				//随机
				if(this.playModel == 1){
